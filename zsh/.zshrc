...
# Function to activate venvs
venv() {
  local envdir="${1:-.venv}"
  if [ -f "$envdir/bin/activate" ]; then
    source "$envdir/bin/activate"
  else
    echo "No virtualenv found at $envdir"
  fi
}
...
# Create a new tmux session with 4 preset windows
tmux-setup() {
    ...
    # Create new session with first window named "terminal"
    tmux new-session -d -s "$session_name" -c "$working_dir" -n "terminal"
    ...
    tmux send-keys -t "$session_name":clod "clod" C-m
    tmux send-keys -t "$session_name":terminal "venv" C-m
